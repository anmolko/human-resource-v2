...
@section('title') Receipt Voucher Trash @endsection
@section('css')
    <style>
        td.details-controls {
            text-align:center;
...
@section('content')
            ...
            <div class="row align-items-center">
                <div class="col">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Receipt Voucher Trash</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                    ...
                    <!-- Receipt voucher Trash Table -->
                    <table id="receipt_entry" class="table table-striped custom-table mb-0 ">
                        <thead>
                        <tr>
                            <th>#</th>
                            <th>Date</th>
                                ...
                                <td class="text-right">
                                    <div class="dropdown dropdown-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item action-restore" href="#"  hrm-restore-action="{{route('receipt-voucher.restore',$trash->id)}}" data-toggle="modal" data-target="#restore_receipt_entry"><i class="fa fa-refresh m-r-5"></i> Restore</a>
                                            <a class="dropdown-item action-per-delete" href="#"  hrm-delete-per-action="{{route('receipt-voucher.remove',$trash->id)}}" ><i class="fa fa-trash-o m-r-5"></i> Delete </a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
    ...
    <!-- Restore Receipt Voucher Modal -->
    @include('admin.modals.receipt_voucher.restore')
    <!-- /Restore Receipt Voucher Modal -->
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function format(mainvalue) {
                ...
                $.each(mainvalue.receipt_particulars_trash, function( index, value ) {
            ...
            var table = $('#receipt_entry').DataTable({
                paging: true,
                searching: true,
                ordering:  true,
            ...
            // for all receipt entry
            $('#receipt_entry tbody').off('click', 'td.details-control');
            $('#receipt_entry tbody').on('click', 'td.details-control', function () {
                ...
                $.post( $url, form_data)
                    .done(function(response) {
                        if(response == 0){
                            swal({
                                title: "Error!",
                                text: "Failed to delete receipt voucher particulars from trash",
                                type: "error",
                                showCancelButton: true,
                                closeOnConfirm: false,
                                showLoaderOnConfirm: true,
