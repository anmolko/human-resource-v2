...
@section('title') View Contra Voucher @endsection
@section('css')
    <style>
        div#contra-single-data-card {
            border: none;
            border-radius: unset;
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Contra Voucher Info</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('dashboard')}}">Main Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{route('account')}}">Account Dashboard</a></li>
                        <li class="breadcrumb-item active"><a href="{{route('contra-voucher.index')}}">Contra Voucher</a></li>
        ...
        <div class="row align-items-center report-groups">
            ...
            <div class="col-auto float-right ml-auto">
                <div class="btn-group btn-group-sm">
                    ...
                    <button class="btn btn-white" onclick="printerDiv('contra-single-data-card')"><i class="fa fa-print fa-lg"></i> Print</button>
                </div>
            </div>
        </div>

        <div class="row" id="contra-single-data">
            <div class="col-md-12">
                <div class="card" id="contra-single-data-card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6 m-b-20">
                                <div class="invoice-details">
                                    <h3 class="text-uppercase ref_id">{{ucwords(@$contra_voucher->ref_no)}}</h3>
                                    <input type="hidden" name="ref_id" id="ref_id" value="{{$contra_voucher->ref_no}}"/>
                                    <ul class="list-unstyled">
                                        <li>Type: <span class="voucher-type">Contra Voucher</span></li>
                        ...
                        <div class="table-responsive">
                            <table id="single-contra" class="ledger-table table table-striped table-hover ">
                                <thead>
                                <tr>
                                    <th>Date</th>
                                    <th>Narration</th>
                                ...
                                <tr>
                                    <?php if(@$theme_data->default_date_format=='nepali'){
                                        $pieces = explode("-", $contra_voucher->date);
                                        echo '<td>'.$pieces[2].' '.getNepaliMonth($pieces[1]).', '.$pieces[0].'</td>';?>
                                    <?php }else if(@$theme_data->default_date_format=='english'){ ?>
                                    <td>{{date('j F, Y',strtotime(@$contra_voucher->date))}}</td>
                                    <?php }else{?>
                                    <td>{{date('j F, Y',strtotime(@$contra_voucher->date))}}</td>
                                    <?php }?>
                                    <td>{{ucwords(@$contra_voucher->narration)}}</td>
                                    <td>{{ucwords(@$contra_voucher->total_amount)}}</td>
                                </tr>
                                <tr>
                                    <td colspan="8">
                                        <table class="table table-striped custom-table mb-0 dataTable no-footer">
                                            ...
                                            <tbody>
                                            @foreach(@$contra_voucher->contraParticulars as $cp)
                                                <?php
                                                if($cp->credit==null){
                                                    $creditname = "-";
                                                }else{
                                                    $creditname = $cp->credit->name;
                                                }

                                                if($cp->debit == null){
                                                    $debitname = "-";
                                                }else{
                                                    $debitname = $cp->debit->name;
                                                } ?>
                                                <tr>
                                                    <td class="text-capitalize">{{$debitname}}</td>
                                                    <td class="text-capitalize">{{$creditname}}</td>
                                                    <td>{{$cp->debit_amount}}</td>
                                                    <td>{{$cp->credit_amount}}</td>
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        var cache_width = $('#contra-single-data-card').width();
        ...
        $(document).on("click", '#generate-pdf', function () {
            ...
            html2canvas($('#contra-single-data-card'), {
                onrendered: function (canvas) {
                    ...
                    doc.save(name+'-contra-details.pdf');
                }
            });
        });
