...
@section('title') View Payment Voucher @endsection
@section('css')
<style>
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Payment Voucher Info</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('dashboard')}}">Main Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{route('account')}}">Account Dashboard</a></li>
                        <li class="breadcrumb-item active"><a href="{{route('payment-voucher.index')}}">Payment Voucher</a></li>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 m-b-20">
                        <div class="invoice-details">
                            <h3 class="text-uppercase ref_id">{{ucwords(@$payment_voucher->ref_no)}}</h3>
                            <input type="hidden" name="ref_id" id="ref_id" value="{{$payment_voucher->ref_no}}"/>
                            <ul class="list-unstyled">
                                <li>Type: <span class="voucher-type">Payment Voucher</span></li>
                        ...
                        <tbody>
                            <tr>
                                <?php if(@$theme_data->default_date_format=='nepali'){
                                    $pieces = explode("-", $payment_voucher->date);
                                    echo '<td>'.$pieces[2].' '.getNepaliMonth($pieces[1]).', '.$pieces[0].'</td>';?>
                                    <?php }else if(@$theme_data->default_date_format=='english'){ ?>
                                <td>{{date('j F, Y',strtotime(@$payment_voucher->date))}}</td>
                                <?php }else{?>
                                <td>{{date('j F, Y',strtotime(@$payment_voucher->date))}}</td>
                                <?php }?>
                                <td>{{ucwords(@$payment_voucher->narration)}}</td>
                                <td>{{ucwords(@$payment_voucher->total_amount)}}</td>
                            </tr>
                            <tr>
                                <td colspan="8">
                                    <table class="table table-striped custom-table mb-0 dataTable no-footer">
                                        ...
                                        <tbody>
                                        @foreach(@$payment_voucher->PaymentParticulars as $pp)
                                            <?php
                                            if($pp->credit==null){
                                                $creditname = "-";
                                            }else{
