            ...
            <div class="page-header">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto float-right ml-auto">
                        <a href="{{route('journal-entry.create')}}" class="btn add-btn" ><i class="fa fa-plus"></i> Add Journal Entry</a>
                    </div>
                    <div class="col-auto float-right ml-auto">
                        <a href="{{route('journal-entry.trash')}}" class="btn add-btn"><i class="fa fa-eye"></i> View Trash</a>
                    </div>
                </div>
            </div>
            <!-- /Page Header -->
                            ...
                            @php($i=1)
                                ...
                                <tr data-child-value="{{$journal_entrie}}">
                                    ...
                                    <?php }else{?>
                                        <td>{{date('j F, Y',strtotime(@$journal_entrie->date))}}</td>
                                    <?php }?>
                                    <td><a href="{{route('journal-entry.print',$journal_entrie->ref_no)}}"> {{ucwords(@$journal_entrie->ref_no)}} </a></td>
                                    ...
                                    <td class="text-right">
                                        <div class="dropdown dropdown-action">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a class="dropdown-item" href="{{route('journal-entry.print',$journal_entrie->ref_no)}}"><i class="fa fa-eye m-r-5"></i> View</a>
                                                <a class="dropdown-item" href="{{route('journal-entry.edit',$journal_entrie->ref_no)}}" ><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                                <a class="dropdown-item action-delete" href="#"  hrm-delete-action="{{route('journal-entry.destroy',$journal_entrie->id)}}" ><i class="fa fa-trash-o m-r-5"></i> Trash</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
            ...
            showLoaderOnConfirm: true,
                ...
                .done(function(response) {
                    if(response == 0){
                        swal({
                            title: "Error!",
                            text: "Failed to move journal particulars into trash",
                            type: "error",
                            showCancelButton: true,
                            closeOnConfirm: false,
                            showLoaderOnConfirm: true,
