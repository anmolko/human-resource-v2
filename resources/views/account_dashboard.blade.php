...
@section('title') Account Dashboard @endsection
@section('css')
...
<!--
        ...
        <div class="row">
            <div class="col-md-6 d-flex">
                <div class="card card-table flex-fill">
                    <div class="card-header">
                        <h3 class="card-title mb-0">Recent Journal Entry</h3>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                                <table class="table table-nowrap custom-table mb-0">
                                    ...
                                    @foreach($journal_entry as $journal)
                                        <tr>
                                        <td><a href="{{route('journal-entry.single',$journal->ref_no)}}">{{$journal->ref_no}}</a></td>
                                            <?php if(@$theme_data->default_date_format=='nepali'){
                                                $pieces = explode("-", $journal->date);
                                                echo '<td>'.$pieces[2].' '.getNepaliMonth($pieces[1]).', '.$pieces[0].'</td>';?>
                                        <?php }else if(@$theme_data->default_date_format=='english'){ ?>
                                            <td>{{date('j F, Y',strtotime(@$journal->date))}}</td>
                                            <?php }else{?>
                                            <td>{{date('j F, Y',strtotime(@$journal->date))}}</td>
                                            <?php }?>
                                        <td>{{ucwords(@$journal->total_amount)}}</td>
                                        <td>
                                            <span class="badge bg-inverse-warning">{{ucwords(App\Models\User::find($journal->created_by)->name)}}</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                    @else
                                        <tr>
                                            <td colspan="4" class="text-center">No journal entry created yet. Click <a href="{{route('journal-entry.create')}}"> here </a> to create one.</td>
                                        </tr>
                    ...
                    <div class="card-footer">
                        <a href="{{route('journal-entry.index')}}" class="capital">View all Journal Entry</a>
                    </div>
                </div>
            </div>
            <div class="col-md-6 d-flex">
